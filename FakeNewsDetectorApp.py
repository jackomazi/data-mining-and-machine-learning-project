...
class FakeNewsDetectorApp:
    ...
    def load_models(self):
        ...
        if self.vectorizer is None or self.model is None or self.scaler is None:
            try:
                self.vectorizer = joblib.load('saved_models\tfidf_vectorizer.pkl')
                self.scaler = joblib.load('saved_models\stylometric_scaler.pkl') 
                self.model = joblib.load('saved_models\lightgbm_tuned.pkl')
            except Exception as e:
